...
class AdvertPlacement(models.Model):
    ...
    advert = models.ForeignKey('ossuo.Advert', related_name='+')
...
# Custom image
class ossuoImage(AbstractImage):
...
@receiver(pre_delete, sender=ossuoImage)
def image_delete(sender, instance, **kwargs):
    # Pass false so FileField doesn't save the model.
    instance.file.delete(False)

class ossuoOSNext(models.Model):
...
class ossuoRendition(AbstractRendition):
    image = models.ForeignKey('ossuoImage', related_name='renditions')
...
# Receive the pre_delete signal and delete the file associated with the model instance.
@receiver(pre_delete, sender=ossuoRendition)
...
class HomePageHero(Orderable, RelatedLink):
    page = ParentalKey('ossuo.HomePage', related_name='hero')
...
class HomePageClient(Orderable, RelatedLink):
    page = ParentalKey('ossuo.HomePage', related_name='clients')
    image = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True,
        blank=True,
...
class StandardPageContentBlock(Orderable, ContentBlock):
    page = ParentalKey('ossuo.StandardPage', related_name='content_block')


class StandardPageRelatedLink(Orderable, RelatedLink):
    page = ParentalKey('ossuo.StandardPage', related_name='related_links')


class StandardPageClient(Orderable, RelatedLink):
    page = ParentalKey('ossuo.StandardPage', related_name='clients')
    image = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True,
        blank=True,
...
class AboutPageRelatedLinkButton(Orderable, RelatedLink):
    page = ParentalKey('ossuo.AboutPage', related_name='related_link_buttons')


class AboutPageOffice(Orderable):
    page = ParentalKey('ossuo.AboutPage', related_name='offices')
...
class AboutPageContentBlock(Orderable):
    page = ParentalKey('ossuo.AboutPage', related_name='content_blocks')
...
class ServicesPageService(Orderable):
    page = ParentalKey('ossuo.ServicesPage', related_name='services')
    title = models.TextField()
    svg = models.TextField(null=True)
    description = models.TextField()
...
class CaseStudyBlock(StructBlock):
    ...
    case_studies = ListBlock(StructBlock([
        ('page', PageChooserBlock('ossuo.WorkPage')),
...
class SignUpFormPageBlock(StructBlock):
    page = PageChooserBlock('ossuo.SignUpFormPage')
...
class BlogIndexPageRelatedLink(Orderable, RelatedLink):
    page = ParentalKey('ossuo.BlogIndexPage', related_name='related_links')
...
class BlogIndexPage(Page):
    ...
    def serve(self, request):
        ...
        if request.is_ajax():
            return render(request, "ossuo/includes/blog_listing.html", {
                'self': self,
...
class BlogPageRelatedLink(Orderable, RelatedLink):
    page = ParentalKey('ossuo.BlogPage', related_name='related_links')
...
class BlogPageTagSelect(Orderable):
    page = ParentalKey('ossuo.BlogPage', related_name='tags')
    tag = models.ForeignKey(
        'ossuo.BlogPageTagList',
        related_name='blog_page_tag_select'
    )


class BlogPageAuthor(Orderable):
    page = ParentalKey('ossuo.BlogPage', related_name='related_author')
...
class BlogPage(Page):
    ...
    streamfield = StreamField(StoryBlock())
    author_left = models.CharField(max_length=255, blank=True, help_text='author who has left ossuo')
...
class ReasonToJoin(Orderable):
    page = ParentalKey('ossuo.JobIndexPage', related_name='reasons_to_join')
    image = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True,
        blank=True,
...
class JobIndexPageJob(Orderable):
    page = ParentalKey('ossuo.JobIndexPage', related_name='job')
...
class WorkPageTagSelect(Orderable):
    page = ParentalKey('ossuo.WorkPage', related_name='tags')
    tag = models.ForeignKey(
        'ossuo.BlogPageTagList',
...
class WorkPageScreenshot(Orderable):
    page = ParentalKey('ossuo.WorkPage', related_name='screenshots')
    image = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True,
        blank=True,
...
class WorkPageAuthor(Orderable):
    page = ParentalKey('ossuo.WorkPage', related_name='related_author')
    author = models.ForeignKey(
        'ossuo.PersonPage',
        null=True,
        blank=True,
...
class PersonPageRelatedLink(Orderable, RelatedLink):
    page = ParentalKey('ossuo.PersonPage', related_name='related_links')
...
class GoogleAdGrantsPageGrantsManaged(models.Model):
    page = ParentalKey('ossuo.GoogleAdGrantsPage', related_name="grants_managed")
    image = models.ForeignKey(
        'ossuo.ossuoImage',
...
class GoogleAdGrantsPageQuote(Orderable):
    page = ParentalKey('ossuo.GoogleAdGrantsPage', related_name="quotes")
...
class GoogleAdGrantsAccreditations(Orderable):
    page = ParentalKey('ossuo.GoogleAdGrantsPage', related_name="accreditations")
    image = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True, blank=True,
        on_delete=models.SET_NULL,
    ...
    def serve(self, request, *args, **kwargs):
        if request.is_ajax() and request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    'ossuo/includes/ad_grant_application_landing.html',
                    {'self': self, 'form': form}
                )
            else:
                return render(
                    request,
                    'ossuo/includes/ad_grant_application_form.html',
                    {'self': self, 'form': form}
                )
        else:
...
class SignUpFormPageBullet(Orderable):
    page = ParentalKey('ossuo.SignUpFormPage', related_name='bullet_points')
    icon = models.CharField(max_length=100, choices=(
        ('ossuo/includes/svg/bulb-svg.html', 'Light bulb'),
        ('ossuo/includes/svg/pro-svg.html', 'Chart'),
        ('ossuo/includes/svg/tick-svg.html', 'Tick'),
    ))
...
class SignUpFormPageLogo(Orderable):
    page = ParentalKey('ossuo.SignUpFormPage', related_name='logos')
    logo = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True,
        blank=True,
...
class SignUpFormPageQuote(Orderable):
    page = ParentalKey('ossuo.SignUpFormPage', related_name='quotes')
...
class SignUpFormPage(Page):
    ...
    email_from_address = models.EmailField(
        ...
        help_text="Anything ending in @ossuo.com is good.")
    ...
    def serve(self, request, *args, **kwargs):
        if request.is_ajax() and request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    'ossuo/includes/sign_up_form_page_landing.html',
                    {
                        'page': self,
                        'form': form,
                ...
                return render(
                    request,
                    'ossuo/includes/sign_up_form_page_form.html',
                    {
                        'page': self,
                        'form': form,
                        'legend': self.call_to_action_text
...
class MarketingLandingPageHeaderRelatedLink(AbstractBaseMarketingLandingPageRelatedLink):
    page = ParentalKey('ossuo.MarketingLandingPage', related_name='header_related_links')


class MarketingLandingPageIntroRelatedLink(AbstractBaseMarketingLandingPageRelatedLink):
    page = ParentalKey('ossuo.MarketingLandingPage', related_name='intro_related_links')


class MarketingLandingPagePageClients(Orderable, RelatedLink):
    page = ParentalKey('ossuo.MarketingLandingPage', related_name='clients')
    image = models.ForeignKey(
        'ossuo.ossuoImage',
        null=True,
        blank=True,
...
class MarketingLandingPageFeaturedItem(Orderable):
    page = ParentalKey('ossuo.MarketingLandingPage', related_name='featured_items')
    related_page = models.ForeignKey('wagtailcore.Page', related_name='+')

    panels = [
        PageChooserPanel('related_page', ['ossuo.BlogPage', 'ossuo.WorkPage'])
    ]
...
class ContactLandingPageRelatedLinkButton(Orderable, RelatedLink):
    page = ParentalKey('ossuo.Contact', related_name='related_link_buttons')


class Contact(AbstractEmailForm):
    intro = RichTextField(blank=True)
    main_image = models.ForeignKey('ossuo.ossuoImage', null=True,
                                   blank=True, on_delete=models.SET_NULL,
                                   related_name='+')
    landing_image = models.ForeignKey('ossuo.ossuoImage', null=True,
                                      blank=True, on_delete=models.SET_NULL,
                                      related_name='+')
...
class GlobalSettings(BaseSetting):
    ...
    headqr_address_title = models.CharField(max_length=255, help_text='Full address')
    headqr_address = models.CharField(max_length=255, help_text='Full address')
    headqr_address_link = models.URLField(max_length=255, help_text='Link to google maps')
    headqr_address_svg = models.CharField(max_length=9000, help_text='Paste SVG code here')
    mbranch_address_title = models.CharField(max_length=255, help_text='Full address')
    mbranch_address = models.CharField(max_length=255, help_text='Full address')
    mbranch_address_link = models.URLField(max_length=255, help_text='Link to google maps')
    mbranch_address_svg = models.CharField(max_length=9000, help_text='Paste SVG code here')
    sbranch_address_title = models.CharField(max_length=255, help_text='Full address')
    sbranch_address = models.CharField(max_length=255, help_text='Full address')
    sbranch_address_link = models.URLField(max_length=255, help_text='Link to google maps')
    sbranch_address_svg = models.CharField(max_length=9000, help_text='Paste SVG code here')

    # Contact widget
    contact_person = models.ForeignKey(
        'ossuo.PersonPage', related_name='+', null=True,
        on_delete=models.SET_NULL,
    ...
    panels = [
        ...
        FieldPanel('headqr_address_title'),
        FieldPanel('headqr_address'),
        FieldPanel('headqr_address_link'),
        FieldPanel('headqr_address_svg'),
        FieldPanel('mbranch_address_title'),
        FieldPanel('mbranch_address'),
        FieldPanel('mbranch_address_link'),
        FieldPanel('mbranch_address_svg'),
        FieldPanel('sbranch_address_title'),
        FieldPanel('sbranch_address'),
        FieldPanel('sbranch_address_link'),
        FieldPanel('sbranch_address_svg'),
...
class MenuItemBlock(StructBlock):
    ...
    class Meta:
        template = "ossuo/includes/menu_item.html"
